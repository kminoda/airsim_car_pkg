...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles)
    {
        ...
        car_ros.takeoff_srvr = nh_private_.advertiseService<airsim_ros_pkgs::Takeoff::Request, airsim_ros_pkgs::Takeoff::Response>(curr_vehicle_name + "/takeoff", 
            boost::bind(&AirsimROSWrapper::takeoff_srv_cb, this, _1, _2, car_ros.vehicle_name) );
        car_ros.land_srvr = nh_private_.advertiseService<airsim_ros_pkgs::Land::Request, airsim_ros_pkgs::Land::Response>(curr_vehicle_name + "/land", 
            boost::bind(&AirsimROSWrapper::land_srv_cb, this, _1, _2, car_ros.vehicle_name) );
        // car_ros.reset_srvr = nh_private_.advertiseService(curr_vehicle_name + "/reset",&AirsimROSWrapper::reset_srv_cb, this);
...
void AirsimROSWrapper::vel_cmd_body_frame_cb(const airsim_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    ...
    tf2::Matrix3x3(get_tf2_quat(car_ros_vec_[vehicle_idx].curr_drone_state.kinematics_estimated.pose.orientation)).getRPY(roll, pitch, yaw); // ros uses xyzw
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_airsim_state(const msr::airlib::CarState& drone_state) const
{
...
void AirsimROSWrapper::drone_state_timer_cb(const ros::TimerEvent& event)
{
    try
    {
        ...
        for (auto& car_ros: car_ros_vec_)
        {
            ...
            car_ros.gps_sensor_msg = get_gps_sensor_msg_from_airsim_geo_point(car_ros.curr_drone_state.gps_location);
